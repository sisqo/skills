...
7. [Languages Where Errors Are Values](#languages-where-errors-are-values)
8. [Don't Return Null](#dont-return-null)
9. [Don't Pass Null](#dont-pass-null)
10. [Error Handling Patterns Summary](#error-handling-patterns-summary)
11. [Common Error Handling Anti-Patterns](#common-error-handling-anti-patterns)
...
Return codes force the caller to check immediately after the call, cluttering the calling code with error-checking logic. (This section applies to exception-based languages; for Go, Rust, and other errors-as-values languages, see [Languages Where Errors Are Values](#languages-where-errors-are-values).)
...
## Languages Where Errors Are Values

"Use exceptions, not return codes" is advice for exception-based languages
(Java, Python, C#, JavaScript). Some languages make errors first-class values
and their idioms take precedence — the *goals* (visible happy path, context on
every error, no silent swallowing) stay the same; the *mechanics* change.

### Go

Errors are ordinary return values. Handle or propagate every one; add context
when crossing an abstraction boundary.

```go
// BAD: swallowed error, no context
data, _ := os.ReadFile(path)

// GOOD: propagate with context, wrap with %w to preserve the chain
...
- Wrap with `fmt.Errorf("...: %w", err)`; test with `errors.Is` / `errors.As`
- Define sentinel errors (`var ErrNotFound = errors.New(...)`) or custom error
  types for conditions callers must distinguish
- Reserve `panic` for unrecoverable programmer errors, not expected failures

### Rust

`Result<T, E>` and `Option<T>` encode fallibility in the type system.

```rust
// GOOD: ? propagates, map_err / custom error types add context
...
- Propagate with `?`; avoid `.unwrap()` / `.expect()` outside tests and truly
  impossible cases (and say why in the `expect` message)
- Use `thiserror` for library error types, `anyhow` for application code
- The equivalent of "don't return null" is built in: return `Option`, not a
  sentinel

### Functional style (also TypeScript, Kotlin, Scala…)

`Either`/`Result` types and railway-oriented pipelines are equally valid ways to
keep error handling out of the happy path. Follow whichever convention the
